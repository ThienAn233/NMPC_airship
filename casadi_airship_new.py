...
for i in range(N):
    ...
    phi_list.append(ca.SX.sym(f'phi_{i}')) # roll (around x-axis)
    theta_list.append(ca.SX.sym(f'theta_{i}')) # pitch (around y-axis)
    psi_list.append(ca.SX.sym(f'psi_{i}')) # yaw (around z-axis)
...
X_scaled = ca.vertcat(*x_list, *y_list, *z_list, *phi_list, *theta_list, *psi_list,
...
# # Normalize the state variables (multiply by a scaling factor and then unnormalize after solving)
# X = X_scaled * ca.vertcat(ca.SX.ones(N)*xf, ca.SX.ones(N)*xf, ca.SX.ones(N)*2000, ca.SX.ones(N)*np.pi, ca.SX.ones(N)*np.pi, ca.SX.ones(N)*np.pi, 
#                    ca.SX.ones(N)*v_max, ca.SX.ones(N)*v_max, ca.SX.ones(N)*v_max, 
#                    ca.SX.ones(N)*np.pi, ca.SX.ones(N)*np.pi, ca.SX.ones(N)*np.pi, 
#                    ca.SX.ones(N)*T_max, ca.SX.ones(N)*lr_max, ca.SX.ones(N)*lr_max)
...
g.append(get_state(N - 1) - ca.vertcat(xf, 0., -2000., 0., 0., 0., 0., 0., 0., 0., 0., 0.))  # End at position (100, 0, 0) with no velocity or rotation 
...
ax.plot([0, xf], [0, 0], [-2000,-2000], color='black', linestyle='--', label='Target Path')
ax.scatter(0, 0, -2000, color='green', label='Start')
ax.scatter(xf, 0, -2000, color='red', label='End')
